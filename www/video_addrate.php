...
$exist = $obj->existVideoRate($_POST["id"], $_SERVER['REMOTE_ADDR']);

if($exist){   // If exist, update the current rate
  $obj->updateVideoRate($_POST["id"], $_POST["value"], $_SERVER['REMOTE_ADDR']);
}
else{       // If not exist, insert a rate
  $obj->insertVideoRate($_POST["id"], $_POST["value"], $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT']);
}

// Update the average rate at `cesar-archive-images-rates`
$obj->updateVideoAvrRate($_POST["id"]);
